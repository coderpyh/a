...
public class Main
{
	...
	static CarButton[] carButton=new CarButton[4];
	static CarMy carMy=null;
	...
	public static void main(String[] args)
	{
		...
		textareaHelp.setBounds(150,200,900,220);
		...
		for(int i=1;i<=3;i++)
		{
			...
			carButton[i]=new CarButton(i,350*i-250);
		...
		imageBoom.setImage(imageBoom.getImage().getScaledInstance(150,150,Image.SCALE_SMOOTH));
	...
	static void clear()
	{
		...
		f.remove(boom);
		f.remove(carButton[1]);
		f.remove(carButton[2]);
		f.remove(carButton[3]);
		if(carMy!=null)
			f.remove(carMy);
		if(obstacle!=null)
